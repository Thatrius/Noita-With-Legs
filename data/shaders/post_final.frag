...
// liquid distortion/refraction effect (calculate distorted texture coordinates for later use) ===============
	...
	if (ENABLE_REFRACTION)
	{
		...
		vec2 liquid_distortion_offset = vec2(
			sin( distortion_mult + (tex_coord.x + camera_pos.x / world_viewport_size.x ) * DISTORTION_SCALE_MULT) * DISTORTION_SCALE_MULT2, 
			cos( distortion_mult + (tex_coord.y - camera_pos.y / world_viewport_size.y ) * DISTORTION_SCALE_MULT) * DISTORTION_SCALE_MULT2 
